...
const Notifications = () => {
  ...
  return (
    <DashboardLayout role="school" userName={profile.name} userSub={profile.district} title="Notifications Activity Center" subtitle={profile.name} notifications={[1, 2]}>
        ...
        <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm p-6 space-y-2">
          <div className="divide-y divide-slate-100">
            {notifications.map((n) => (
              <div
                key={n.id}
                className={`p-4 rounded-2xl flex items-start gap-4 transition-colors ${!n.read ? "bg-blue-50/50" : "hover:bg-slate-50/60"
                  }`}
              >
